...
def _transcribe(audio_path, word_timestamps, language=None):
    model = _get_model()
    segments, info = model.transcribe(audio_path, word_timestamps=word_timestamps, language=language)
    out = []
    for s in segments:
    ...
    while True:
        ...
        try:
            if op == "ping":
                ...
                result = _transcribe(msg.get("audio_path"), bool(msg.get("word_timestamps", True)), msg.get("language"))
