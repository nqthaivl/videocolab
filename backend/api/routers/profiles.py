...
async def create_profile(
    ...
    if kind == "clone":
        ...
        # Transcribe reference audio at creation if ref_text is empty
        if not (ref_text or "").strip():
            from services.asr_backend import transcribe_reference
            try:
                import asyncio
                transcribed = await asyncio.get_running_loop().run_in_executor(
                    None, transcribe_reference, audio_path
                )
                if transcribed:
                ...
                logging.getLogger("omnivoice.profiles").warning(
                    "ASR transcription failed during profile creation: %s", asr_err
                )
    else:
