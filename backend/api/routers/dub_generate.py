                ...
                if profile_id:
                    ...
                    if row:
                        if row["is_locked"] and row["locked_audio_path"]:
                            ...
                            ref_audio = os.path.join(VOICES_DIR, row["ref_audio_path"]) if row["ref_audio_path"] else None
                            ref_text = row["ref_text"]
                            used_seed = row["seed"]
                            
                        # If reference audio is set but ref_text is empty/falsy, transcribe it on-the-fly
                        if ref_audio and not (ref_text or "").strip():
                            try:
                                from services.asr_backend import transcribe_reference
                                logger.info("Transcribing reference audio on-the-fly for profile %s: %s", profile_id, ref_audio)
                                transcribed = transcribe_reference(ref_audio)
                                if transcribed:
                                    ref_text = transcribed
                                    logger.info("Transcribed successfully: %s", ref_text)
                                    # Save back to database so we don't transcribe again next time!
                                    with db_conn() as conn:
                                        conn.execute("UPDATE voice_profiles SET ref_text=? WHERE id=?", (ref_text, profile_id))
                            except Exception as asr_err:
                                logger.warning("On-the-fly reference transcription failed: %s", asr_err)
